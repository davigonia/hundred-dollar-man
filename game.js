...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayOwnedPets() {
        ...
        ownedPets.forEach(pet => {
            ...
            petElement.innerHTML = `<span class="pet-emoji" data-pet="${pet}">${petEmoji}</span>`;
...
function createGameCompletePopup() {
    ...
    continueBtn.className = 'popup-btn';
...
function showQuestion() {
    ...
    // Shuffle answers and track the new position of the correct answer
    ...
    // Show coin indicator and the question
    questionText.textContent = `${coinIndicator} ${currentQuestion.question}`;
    ...
    answerButtons.forEach((button, index) => {
        ...
        // Store the original index as a data attribute for checking correctness
...
function checkAnswer(selectedAnswer) {
    const selectedButton = answerButtons[selectedAnswer];
    ...
    if (isCorrect) {
        selectedButton.classList.add('correct'); // Add 'correct' class instead of 'incorrect'
        correctSound.play();
        score += 10;
        ...
        selectedButton.classList.add('incorrect'); // Only add 'incorrect' class when wrong
        incorrectSound.play();

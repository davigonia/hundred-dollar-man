...
const coinValues = [1, 2, 3, 5, 10, 15, 20, 30, 40, 50]; // Coin values for each question
...
function startGame(category) {
    ...
    updateProgressTracker(currentQuestionNumber);
...
function showQuestion() {
    ...
    // Show only coin indicator and question (removed "Question x/10" prefix)
...
function checkAnswer(selectedAnswer) {
    ...
    if (isCorrect) {
        ...
        if (safetyNetLevels.includes(currentQuestionIndex)) {
            securedCoins = coins; // Secure current coins
        ...
        // After handling correct answer and before showing next question
        ...
        // Reset progress tracker on wrong answer
...
// Add this function to create the progress tracker
function createProgressTracker() {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        indicator.dataset.level = i;
        indicator.textContent = i;
        
        // Add safety net indicator for questions 3 and 7
        if (i === 3 || i === 7) {
            const safetyIcon = document.createElement('div');
            safetyIcon.className = 'safety-net-indicator';
            safetyIcon.innerHTML = '★'; // Star symbol for safety net
            indicator.appendChild(safetyIcon);
        }
...
// Add this function to update the progress tracker
function updateProgressTracker(questionNumber) {
    ...
    for (let i = 1; i < questionNumber; i++) {
        const indicator = document.querySelector(`.question-indicator[data-level="${i}"]`);
        if (indicator) indicator.classList.add('completed');
    }
    
    // Mark current question
    const currentIndicator = document.querySelector(`.question-indicator[data-level="${questionNumber}"]`);
    if (currentIndicator) currentIndicator.classList.add('current');
    
    // Update current coins display
    document.getElementById('current-potential').textContent = coins;
    
    // Update secured coins based on safety nets
    document.getElementById('secured-coins').textContent = securedCoins;
} 